...
function CustomeCaseCard(row) {
  ...
  function shareQRCards(){
    html2canvas(document.querySelector('#printQRDiv'), {
        ...
        window.plugins.socialsharing.share(null, null, canvas.toDataURL('image/jpeg'), null)
      }).catch(error=>{
        setGlobalSpinner(false)
      });
  }
        ...
        >
          ...
          {/* alertsdetailholder */}
            ...
            <p className="alerts_profilebtn_holder caselistviewbtnholder myClientsCaseList">
              ...
              <Button variant="contained" color="primary" disableElevation className="infoviewmorebtn ml-10px" onClick={getPatientCard}>
          ...
          <CardActions disableSpacing className="cardactionfooter">
            ...
            {clientType && clientType.toLocaleLowerCase() == 'index' ?
              <>
                ...
                <button className="caseSearchButton" type="button" onClick={addContcat}>
                <AddIcon/>
                </button>
              </> :
              <> </>}
...
export default connect(null, {setTrackedEntityInstanceAction})(CustomeCaseCard);