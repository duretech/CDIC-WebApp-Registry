...
function Cases(){
            ...
            try{
                res.data.headers.map(ele => {
                    if(ele.column && progarmData){
                        let fieldBO = _.find(progarmData.trackedEntityAttributes,{displayName:ele.column})
    ...
    return (            
        <section className="searchcustombg searchtabmaindiv myclientspage myclientspagelist"
                style={{
                    // backgroundColor: '#fff',
                    flexGrow: 1,
                    padding: 0,
