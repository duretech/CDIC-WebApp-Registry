...
function CustomeCaseCard(row) {
  ...
  function openFollowup(){
    let transferoutstage = row.metaData.programs[0].programStages.find(obj => obj.description == "Follow up")
    console.log("transferoutstage ",transferoutstage,row.metaData.programs[0].programStages)
    const activeCaseDetails = {
      'trackedEntityInstance': row.row[0],
      'enrollmentId': "",
      "stageuid": transferoutstage && transferoutstage.id ? transferoutstage.id : ''
    }
    const linkContact = {
      "enabled": false,
      "linkTrackedEntityInstance": row.row[0]
    ...
    OfflineDb.setDataIntoPouchDB('redirectedFromFollowUp', true)
    history.push('/layout/registration')
  }
        ...
        >
          ...
          {/* alertsdetailholder */}
            ...
            <p className="alerts_profilebtn_holder caselistviewbtnholder myClientsCaseList">
              ...
              <Button variant="contained" color="primary" disableElevation className="infoviewmorebtn ml-10px" onClick={()=>openFollowup()}>
                <ArrowForwardIcon /> <span className="ml-10px">{t("Follow up")}</span>
              </Button>
              { 
